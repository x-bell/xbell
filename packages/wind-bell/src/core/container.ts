...
import { WindBellConfig, IBatchData, PropertyKey, MultiEnvData } from '../types';
...
class Container {
  ...
  protected bellConfig!: WindBellConfig;
  ...
  public getValidCase(constructor: Function): ICase[] {
    return this.targetCaseName
      ? this.getCases(constructor).filter(c => c.caseName === this.targetCaseName)
      : this.getCases(constructor);
  }
  ...
  public async runAllGroup(envConfig: EnvConfig) {
    const exportGroups = this.groups.filter(group => this.isValidGroup(group));
    for (const [groupIndex, group] of Array.from(exportGroups.entries())) {
      const cases = this.getValidCase(group.constructor);
      if (!cases.length) {
        console.log(`该组${String(group.groupName)}暂无case`);
        console.log('group.target::', group.constructor);
      }
      try {
  ...
  public async runGroup(target: IGroup, groupIndex: number, envConfig: EnvConfig) {
    const cases = this.getValidCase(target.constructor);
  ...
  protected async _runCase(constructor: Function, propertyKey: PropertyKey, ctx: Context, {
    ...
    const customDecoratorCalbacks = (Reflect.getMetadata('', constructor, propertyKey) || []) as CreateDecorateorCallback[];
    const reverseCustomDecoratorCallbacks = customDecoratorCalbacks.reverse()

    const batchData = this.getBatchData(constructor, propertyKey, ctx.envConfig.ENV);

    const runNextDecoratorCallback = async () => {
      const decoratorCallback = reverseCustomDecoratorCallbacks.pop()
      if (decoratorCallback) {
        await decoratorCallback(ctx, runNextDecoratorCallback)
      } else {
        // 执行 case，判断是否批量数据
        if (batchData) {
          if (!batchData?.length) {
  ...
  public isValidGroup(group: IGroup): boolean {
    const cases = this.getValidCase(group.constructor);
    // TODO: 环境
    return this.isExport(group.constructor) && !!cases.length && (this.targetGroupName ? group.groupName === this.targetGroupName : true);
  }
  ...
  public async initContext(envConfig: EnvConfig, {
    ...
    const browser = await chromium.launch({
      headless: !!this.bellConfig.headless,
    });
    ...
    const ctx = new Context(envConfig, browser, page, expect);
    return ctx;
  }

  public async addCustomClassDecorator(cls: Function, callback: CreateDecorateorCallback, parameters: any[]) {
  ...
  public async addCustomMethodDecorator(cls: Function, propertyKey: PropertyKey, callback: CreateDecorateorCallback, parameters: any[]) {
  ...
  public async runAllEnvs() {
    ...
    if (!runEnvs?.length) {
      console.log('启动环境未定义，请配置 bell.config.ts 文件中 runEnvs 属性');
      return;
    }
  ...
  public loadWindBellConfig(): WindBellConfig {
  ...
  public initConfig(
    ...
    { groupName, caseName }: Partial<{ groupName: string, caseName: string }>
  ) {
    this.projectDirPath = projectDirPath;
    this.targetCaseName = caseName;
    this.targetGroupName = groupName;
    this.bellConfig = this.loadWindBellConfig();
  }

  public initReporter(env: EnvConfig['ENV']) {
    const groupRecord = this.groups
      .filter(group => this.isValidGroup(group))
      .map(group => {
        const cases = this.getValidCase(group.constructor);
    ...
    const reporter = new Reporter(groupRecord, env!);
    return reporter;
  }
}
