...
- `received` <type>Mock</type> The mock function
- `nthCall` <type>number</type> Nth call
- `args` <type>any[]</type> Expect arguments

**Returns** <type>void</type>

Use `toHaveBeenNthCalledWith` to ensure that the mock function is called the nth time with the specified arguments.
...
### toHaveReturned
**Signature: expect(received).toHaveReturned(returnValue)**

- `received` <type>Mock</type> The mock function
- `returnValue` <type>any</type>

**Returns** <type>void</type>

Use `toHaveReturned` to ensure that a mock function was called with specific return value. The return value are checked with the same algorithm that `toEqual` uses.
...
test('toHaveReturned', ({ expect, fn, spyOn }) => {
  ...
  expect(mockAdd).toHaveReturned(2);
  expect(someOne.makeSomething).toHaveReturned('something');
});
```
### toHaveReturnedTimes
...
**Signature: expect(received).toBeVisible()**
